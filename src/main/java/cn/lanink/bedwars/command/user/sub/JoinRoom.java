package cn.lanink.bedwars.command.user.sub;

import cn.lanink.bedwars.BedWars;
import cn.lanink.bedwars.arena.BaseArena;
import cn.lanink.bedwars.command.BaseSubCommand;
...
public class JoinRoom extends BaseSubCommand {
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        if (!this.bedWars.getArenas().isEmpty()) {
            ...
            if (args.length < 2) {
                ...
                if (arenas.size() > 0) {
                    BaseArena arena = arenas.get(BedWars.RANDOM.nextInt(arenas.size()));
                    arena.joinRoom(player);
                    sender.sendMessage("§a已为你随机分配房间！");
                    return true;
                }
                ...
                if (s.length == 2 && "mode".equals(s[0].toLowerCase().trim())) {
                    ...
                    LinkedList<BaseArena> arenas = new LinkedList<>();
                    for (BaseArena room : this.bedWars.getArenas().values()) {
                    ...
                    if (arenas.size() > 0) {
                        BaseArena room = arenas.get(BedWars.RANDOM.nextInt(arenas.size()));
                        room.joinRoom(player);
                        sender.sendMessage("§a已为你随机分配房间！");
                        return true;
                    }
                    ...
                    String world = args[1];
                    BaseArena arena = this.bedWars.getArenas().get(world);
                    if (arena != null) {
