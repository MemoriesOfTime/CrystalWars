...
public class PlayerJoinAndQuit implements Listener {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        ...
        if (player != null && BedWars.getInstance().getRooms().containsKey(player.getLevel().getName())) {
            BedWars.getInstance().getServer().getScheduler().scheduleDelayedTask(new Task() {
                @Override
                public void onRun(int i) {
                    if (player.isOnline()) {
                        Tools.rePlayerState(player ,false);
                        SavePlayerInventory.restore(player);
                        player.teleport(BedWars.getInstance().getServer().getDefaultLevel().getSafeSpawn());
                    }
                }
            }, 120);
        }
    ...
    public void onPlayerQuit(PlayerQuitEvent event) {
        ...
        for (Room room : BedWars.getInstance().getRooms().values()) {
    ...
    public void onPlayerTp(PlayerTeleportEvent event) {
        ...
        if (!fromLevel.equals(toLevel)) {
            LinkedHashMap<String, Room> room =  BedWars.getInstance().getRooms();
            if (room.containsKey(fromLevel) && room.get(fromLevel).isPlaying(player)) {
                event.setCancelled(true);
                player.sendMessage(BedWars.getInstance().getLanguage().tpQuitRoomLevel);
            }else if (!player.isOp() && room.containsKey(toLevel) &&
                    !room.get(toLevel).isPlaying(player)) {
                event.setCancelled(true);
                player.sendMessage(BedWars.getInstance().getLanguage().tpJoinRoomLevel);
            }
        }
    }
